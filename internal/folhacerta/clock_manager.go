...
func (c *_FolhaCertaClockManager) Query(ctx context.Context) (qisno.ClockInfo, error) {
	...
	if !resp.IsSuccess() {
		return qisno.ClockInfo{}, fmt.Errorf("received invalid status code %d", resp.StatusCode())
	}
	result := resp.Result().(*CarregarDiaResponse)
	if !result.Success {
		return qisno.ClockInfo{}, fmt.Errorf("request failed: %s", result.Error)
	}
